...
class WiredValidator {
    ...
    void validateGraph() {
        ...
        for (Map.Entry<String, WiringModel.Component> component :
            ...
            for (int index = 0; index < parameters.size(); index++) {
                ...
                else if (parameter.internal()) bind(parameter, parameterAnchor(element, index));
                else bindExternal(parameter);
            }
        }
    ...
    // implementation makes the choice unambiguous and stay silent otherwise
    private void bindExternal(WiringModel.Parameter parameter) {
        List<String> implementations = implementations(parameter.erasedClass(), parameter.qualifier());
        if (implementations.size() == 1)
            model.addBinding(parameter.erasedClass(), parameter.qualifier(), implementations.get(0));
    }
    ...
    private void collect(TypeElement component, ExecutableElement constructor) {
        ...
        for (VariableElement parameter : constructor.getParameters()) {
            WiringModel.Parameter collected = collectParameter(parameter);
    ...
    private WiringModel.Parameter collectParameter(VariableElement parameter) {
        ...
        return new WiringModel.Parameter(
                erased, type.toString(), qualifier(parameter), isSingleton(type), isInternal(type), optional);
    }
